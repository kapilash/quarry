...
nl :: Lexer QToken
nl = do
  (space <|> csComments <|> newLine)
  return QWhitespace
 where space = matches (\x -> x == ' ' || x == '\t')  >> return ()

csToken = do
  c <- peek
  if isDigit c then csDouble
    else if (isAlphaNum c) || ( c == '_') || (c == '$') then csIdent
         else if c == '\'' then charLiteral
              else if c == '"' then strLiteral
                   else if (c == ' ') || (c == '\t') then do { anyChar; return QWhitespace}
                        else if (c == '\r') || ( c== '\n') then (newLine >> return QNewLine)
                             else csOper            
eof :: Lexer QToken
eof = Lexer eof'
...
counter :: Int -> QToken -> Int
counter i QWhitespace = i + 1
counter i _       = i

runSingleInput ::  LexInput -> IO ()
runSingleInput  inp@(LexInput i l c) =
    case (unLex (some csToken)) inp of
      (Left s, LexInput t _ _)  -> do { print "fail" ;putStrLn s; putStrLn t; return ()}
      (Right a, LexInput r l c) -> do
                            print $ foldl counter 0 a
                            putStrLn r
                            print (l,c)
...
main = do
  ...
  runSingleInput (LexInput fc 1 1)
